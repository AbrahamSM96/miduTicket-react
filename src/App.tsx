...
function App() {
  ...
  const handleClickTecnologies = (event) => {
    const { target } = event;
    console.log(event, 'targeet');
  };
  useEffect(() => {
    // const wrapper = refWrapper.current;
    // const ticket = refTicket.current;
    ...
    //   // ticket.style.transition = `transform .5 ease-in-out`
    ...
    const updateCustomProperties = (tech) => {
      const { borderColor, backgroundColor, textColor } =
        stylesIcons[tech] ?? stylesIcons.javascript;
    ...
    document.querySelectorAll('#select button').forEach((button) =>
      button.addEventListener('click', (event) => {
        ...
        const technologieSelected = currentTarget.getAttribute('data-tech');
        ...
        updateCustomProperties(technologieSelected);

        // document.querySelector('#card-icon').innerHTML = currentTarget.innerHTML;
        const newFlavourSelect = techs.find(({ tech }) => tech === technologieSelected)?.Icon;
        console.log(newFlavourSelect, 'newFlavourSelect');
        console.log(technologieSelected, 'tech');
